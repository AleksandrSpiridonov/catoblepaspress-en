;(() => {
  ...
  host.insertAdjacentHTML(
    ...
    <div class="viewer-wrap">
      <div class="viewer-meta">
        <strong id="v-title">Loading…</strong>
        <span id="v-count">0 / 0</span>
      </div>
      <div id="v-box" class="viewer-box">
        <button id="v-fs" class="viewer-nav viewer-fs" aria-label="Full screen">⛶</button>
        <button id="v-prev" class="viewer-nav viewer-prev" aria-label="Previous">‹</button>
        <button id="v-next" class="viewer-nav viewer-next" aria-label="Next">›</button>
      </div>
    </div>
  `,
  )
  ...
  const manifestUrl =
    qs.get("manifest") ||
    (window.VIEWER_CONFIG?.manifestByIssue
  ...
  function render() {
    ...
    box.appendChild(el)
    titleEl.textContent = manifest.title || `Issue ${issue}`
  ...
  fetch(manifestUrl)
    ...
    .then((data) => {
      if (!Array.isArray(data.pages) || data.pages.length === 0) throw new Error("Empty manifest")
      manifest = data
      render()
    })
    .catch((err) => {
      ...
      box.insertAdjacentHTML(
        "beforeend",
        `<div style="padding:12px;color:#fff">Could not load the issue: ${err.message}</div>`,
      )
    })
})()
